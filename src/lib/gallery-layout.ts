 ...
 *  matches the widest the cell can grow to, at 2× density. */
export function galleryCell(image: ImageMetadata) {
  const ratio = image.width / image.height;
  const widest = Math.round(GALLERY_SHORT_PX * Math.max(ratio, 1) * GALLERY_STRETCH);
  return {
