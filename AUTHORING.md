...
piece whose frames name the place, and the page grows. A borrowed photograph stays with its home
piece, never counted twice. `/places/` lists the places as cards, most
...
yet get a note, not a failure: no page, no card, and their frames show no place, so a place can be
declared ahead of its first outing. A gallery-root photograph cannot
