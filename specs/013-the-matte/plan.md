...
- **The four surfaces, and what "off" is.** Each surface's decision is
  ...
  | the image page's stage                                         | form V+H on `.image-frame`; the compare figure (form W) with `padding: var(--mat)`; `.compare-frames { gap }` and `.compare-note { margin }` at `calc(var(--baseline) / 2)` on either branch (amended 2026-09-17, decision review, T1104a) — neither is a mat, and `--mat` carries `100%`, which resolves against the reader's own containing block: on a child of the padded figure that is the content box, `2sq/(1+2sq)` short of the figure's mat (7.4% at share 0.06 on a 3:2), and as a grid `row-gap` it reads the height axis | `.image-frame { --mat: 0px }`, delete its `padding`/`background`; the compare's `padding`/`background` deleted; drop `.compare` from the form W list                                                                                                                                                                                                                                                                                                                   |
...
- **The compare's note margin and no-JS frames gap are prose spacing,
  not mats** (`calc(var(--baseline) / 2)`, the off branch's value, on
  both branches; decision review at the top tier, 2026-09-17, landed as
  T1104a): `--mat` is applied only as `padding`, because its `100%`
  resolves against the reader's containing block — on the padded
  figure's children that is the content box, and for a grid `row-gap`
  the height axis. A second expression of form W on the children would
  break the single source; accepting the shortfall would document an
  approximation that varies with share and ratio. The look changes on
  one page: the note sits half a baseline below the frames rather than
  a mat's width.
