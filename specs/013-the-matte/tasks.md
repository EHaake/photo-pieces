...
- [x] **T1104** — The gate's values and each surface's presence.
...
_T1104 record (2026-09-17, implementer at opus):_ `sh scripts/verify.sh` — `87 page(s) built in 1.41s`, `[check-no-gps] 741 images scanned in dist/ — no GPS metadata.`, `[check-no-dev-routes] no dev routes in dist/.`, `BUILD EXIT 0`, `CHECK EXIT 0` (0/0/0), `Test Files 15 passed (15)`, `Tests 324 passed (324)` (323 + the gate identity case), `TEST EXIT 0`; Prettier clean. `:root` → `--mat-share: 0.06`, `--mat-min: 0.25rem`, `--mat-max: 2.5rem`, comment dated to the gate; nothing else in the stylesheet changed (every surface on). `git diff main -- src/content.config.ts src/lib/images.ts src/lib/image-meta.mjs src/lib/image-set.ts src/lib/gallery-layout.ts obsidian-plugin/` empty. Geometry (Firefox 155.0.1 headless via BiDi, dev server; expected = clamp(4, 0.06·σ, 40) from the measured short side): **1512×982** `/pieces/vocabulary-sampler/` — 3:2 single 24.667 (exp 24.683; the gate's 24.667), square inset 23.567 (23.572), wide 40.0 (ceiling), diptych member 12.183 (12.203), weighted 16.25 / 11.75, grid 12.183 / 17.633, aside 16.05, row 18.2 / 12.583 — three sizes, three values in size order; match="height" diptych 16.4 on both members (matched height 273.817 → 16.429), triptych 11.05 on all three; held portrait figure height 883.85 vs `100svh − 2·hold-margin` 883.80 (height-bound, tight), mat 32.733 (32.735); held landscape width-bound, mat 23.75 (23.756); pause `--frame-w` 1346.48 = measured 1346.483, `--frame-h` 483.35 = 483.333, mat 25.883 (25.894); all four paddings equal on every frame. Packed rows: galleries and the place wall (target 324.05) 19.443 on every cell, one short side per row (410.733 / 410.683; 437.517 / 437.45), one padding; related strip (target 120) 7.2 on all six, row short side 146.8. `/places/` cards 13.867 (13.856). Stage 40.0 (ceiling), img 1080×719.72, img + 2m = 1160 = `--avail-w` (tight on width), height ≤ 847.1; quiet view: frame background `oklch(1 0 0)` = `#ffffff` (Firefox serialises in oklch; resolved through a canvas round-trip), mat 40.0. **1280×1440** — the reading column is the same width so the same shares: single 24.667, inset 23.567, wide 40, match-height 16.4 / 11.05 (AC 2's "same share on both screens"); held both width-bound, mats 40 and 32.05; pause `--frame-w` 1097.14 = 1097.15, `--frame-h` 393.846 vs 394.10 (Δ 0.25); galleries and place wall (target 422.4) 25.344 everywhere; cards 13.867; stage 40 (ceiling), quiet `#ffffff`; related strip 7.2. **375×812** — single 12.133 (12.159), inset 16.067 (16.072), wide 13.333 (13.344) — three values in size order; diptych 12.133 / 17.567; held 12.133 and 17.567 (width-bound); pause `--frame-w` 311.429 = 311.433, `--frame-h` 111.795 vs 111.633, mat 5.983; galleries and place wall (target 280) 16.8, one short side and padding per row; cards 12.133; stage 12.988, quiet `#ffffff`; related strip (target 96) 5.76 on all six — the 4px floor is not binding at share-60 (T1103: it bites only below share-25); the smallest mat anywhere at 375 is the stacked triptych's 5.3. `scrollWidth ≤ clientWidth` on the galleries, place, places and image pages at all three viewports; on the vocabulary sampler 1505 > 1497 (1273 > 1265; 368 > 360) — identical with T1101's inert tokens injected, caused by the fullbleed and half-bleed figures at `100vw` against headless Firefox's 15px classic scrollbar (overlay scrollbars on a Mac make them equal); not the gate's values. Three deltas above 0.5px, all documented limitations: the half-bleed wide at 1280 (36.033 vs 37.487) and 375 (13.017 vs 13.55) — one mat on the bled side; match-height members stacked at 375 (7.967 / 5.3) keep the block's mat; the stage at 375 12.988 vs 12.397 from the rendered side — `--avail-w` probes 350.667 (the scrollbar inside `100vw`), exact given its input. Nothing unmeasured. Deviations: the quiet colour is recorded as `#ffffff` via canvas, not the literal `rgb(255, 255, 255)`; the Firefox launch recipe now needs `open -na /Applications/Firefox.app --args --headless --no-remote --profile <scratch> --remote-debugging-port=9222 --remote-allow-hosts=localhost,127.0.0.1` (the direct binary prints "Could not find profile folder"). Findings: the quiet state persists in `sessionStorage` and hides `.image-body`, so a driver must clear it before measuring the related strip; the Mattes comment's "where `:root` stands until spec 013's gate" sentence is stale (T1106's); at share-60 the ceiling bites on `wide` and the stage at 1512 and 1280. Read beyond the bundle: the image page's quiet script region.
