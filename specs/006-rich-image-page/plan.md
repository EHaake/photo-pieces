...
- The frame goes through `<Image>` like every image and the 004
  ...
  GPS so the post-build scan has a real block to find if a private
  raster ever reaches the output untouched — which it did once, at
  T403 (see Corrections). (The first draft here claimed the GPS also
  guarded the source-width, source-format passthrough shape; the
  pre-merge sweep showed it can't: the compare requests 1400px of a
  1600px frame, a real transform whatever the format. Withdrawn.)
...
data-quiet-toggle>` the script un-hides; `html[data-quiet]` hides
  ...
  (the page value read off the attribute, not repeated) so the browser
  picks a larger srcset candidate; Escape or any click exits (T409a —
  the photographer's amendment at the visual gate; the first cut kept
  the light ground and the reading width, and "didn't really do
  anything"). Between image pages the state lives in `sessionStorage`
  and is re-applied at `astro:after-swap`: the router replaces
  `<html>`'s attributes on every swap, so nothing "persists" on the
  element itself; the script clears it on any other page.
...
- `where-the-fog-lets-go/_land-b.jpg`: the camera's frame for the demo
  ...
  **4:3** variant of `land-b` (a different crop, for the letterbox
  path) carrying GPS, for the scan to find if it ever ships untouched.
...
- **Pre-merge review amendments (T411, skeptical reviewer: sound with
  changes).** Record corrections: the stage `<Image>` comment claimed
  its width was never the source's — it usually is; what keeps the
  request a real transform is Astro's webp output (a source-format
  request at source width is the passthrough shape, guarded in
  `og.ts` where the format is jpg on purpose). The GPS "tripwire"
  claim was withdrawn (above). The transform refuses a private raster
  in its own check (`rejectPrivateSrc`, ahead of the `alt=""`
  exemption that would skip `imagePageUrl`), so `parseImagePath`'s
  private branch is a guard for other callers, not the transform's
  path — both now speak through one exported `privateMessage()`, and
  the unit test is named for what it asserts. Holes closed: a piece
  `cover` naming a private raster now fails the build (the frontmatter
  half of "a piece that references one fails"); `place` and `time` are
  trimmed like every other sidecar field. Drift removed: the set's
  storage key and key format live in `src/lib/image-set.ts`, imported
  by the layout script, the page script, and the page markup; the
  compare's two split expressions read one `--split-pct`. Layout: the
  story's un-mute no longer rides on stylesheet order
  (`.sec-story.prose p`); the related strip keeps its stretch cap below
  720px (a lone thumbnail no longer fills the column) and its `sizes`
  describes the narrow short side, not the gallery's one-per-row
  94vw. Noted, not changed: the section order and the baseline
  equivalence were verified by one-off scripts at T406, not standing
  tests; `sets` construction is registry-shell code verified by the
  T405 dump, the 004 pattern.
