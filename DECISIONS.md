...
Spec 015 (2026-09-19) reversed 013's "every surface" answer, and this
decision still stands: the site still applies the one mat it has, and
still never bakes one into a file. What changed is _where_ it applies —
the image page's stage and the quiet view that grows out of it, plus a
piece's pause frame until the pause's own spec; every other surface is
the photograph on the ground, edge to edge. That the change was one
stylesheet edit and no re-export is this decision paying off a second
time; see "Spec 015: the hero mat" below.
...
left this tone alone — at that gate the photographer read the mattes as
possibly not standing out enough, this warming as possibly not having
gone far enough, and asked for a spec that tries a range of tones.

**Superseded by spec 015 (2026-09-19).** The reason this warming existed
— white mats that have to read against the page — is gone from every
page but the image page's, once spec 015 took the mat off every other
surface. Re-judged on the unmatted pages with spec 014's switch, on both
screens, the ground went back to **paper**: `--color-bg` to
oklch(0.99 0.003 100), the pre-003 white, with surface/soft/line and
line-strong at 014's fixed steps from it (0.967 / 0.942 / 0.882 / 0.742)
and the muted text unchanged. The derived copies moved by the same rule:
the OG route's bg hex is #fcfcfa and `public/og.jpg` was regenerated by
`npm run og`. The stage's white mat on paper reads faintly outside the
quiet view — accepted, with the stage's darker field its own spec
(`ROADMAP.md`). See "Spec 015: the hero mat" below.
...
**Reversed at spec 015 (2026-09-19).** The two paragraphs above record the
answer as it was given; they are no longer the site. Looking at spec 014's
ground candidates on the real photographs, the photographer reached a
different conclusion about the mats themselves: in the reading flow of a
piece and in the packed rows of a gallery the frames look better without
them, and the mat is a hero treatment for the image page's stage and its
quiet view alone (the pause frame excepted until its spec). With the mats
gone from those pages the ground lost the job the 003 warming gave it and
was re-judged: paper. The three tokens, the per-form rule, the stage's
form V+H and the sampler all stand — a later spec turns a surface back on
by restoring its form — and so does every fit the rule keeps exact at a
zero mat. See "Spec 015: the hero mat" below.
...
## Spec 015: the hero mat

Spec 013 asked whether the site wants mats at all and where, and answered
"everywhere, at 6% of the frame". Spec 014 then set out to choose a ground
for those mats to sit behind, and at its gate, with the candidates on the
real photographs, the photographer reached a different conclusion about the
mats themselves. In his words: "in the galleries and pieces, I think it
looks better to remove them." The mat is a hero treatment — it belongs on
the one presentation where a single photograph is the whole point, the image
page's stage and the quiet view that grows out of it, and nowhere else. Spec
015 made that change, then answered the question it reopened: what ground
the site wants when nothing but the stage needs a wall to read against.

**Off is `--mat: 0px` in place of the form where a formula still reads the
mat, and simply nothing where only the padding did** — not a per-surface
token, not the tokens zeroed on a wrapper, not a form left computing beside
an override. Two declarations of `--mat` on one element are a cascade, not
a value; a form left live on a surface whose padding is gone would still put
`2m` into the packed row's basis and the held box and break the very fits
the spec protects; and the tokens cannot be zeroed per surface, because the
test that keeps them a single source forbids declaring them outside `:root`.
So the held figure and the packed cell declare `--mat: 0px` and their
`max-width`, `flex` and cap formulas keep their `+ 2 * var(--mat)` term —
now zero, from the same property they always read, which is what keeps the
packing, the held fit and the srcset hints exact. The matched pair's per-form
rule (P) and the held figure's `--q` are deleted outright: nothing read them
but the mat itself. Where nothing computes (the cover card, the compare
figure, the reading-flow frames) the padding and background are simply
gone. This is spec 013's own presence table, accepted at its sign-off;
turning a surface back on is the form restored to its element plus
`padding: var(--mat)` and the background — one edit per surface. The
compare's letterbox fill and slider divider keep `--color-matte`: they are
the compare's own device inside its box, not a field around the photograph,
and white unless the photographer says otherwise.
...
own rules, the piece page's script and the transform are byte-identical
against `main`, and the anchor's computed padding, background and box were
read before and after — the same numbers at three viewports. One mechanism
changed with no change in effect: narrowed to one selector the list rules
lose the specificity that used to beat `.piece-block a.image-link`'s
`background: none`, and the white now wins by source order, which a test
pins. A piece with a pause therefore carries one matted frame until that
spec lands. The held frame, by contrast, is reading-flow and unmats with
the rest — the spec author's lean, accepted.

**The stage's darker field is deferred, with its leans recorded.** The
photographer wants the stage in a field much darker than the page, near the
quiet dark, giving way to the light ground as the reader scrolls into the
words. Two leans go with it to that spec (`ROADMAP.md`, "The stage's
darker field", likely the pause's): the field is the stage's own background
and scrolls away with it, no script; and it ends at the stage's edge, the
header and the frame nav on the page ground. Until then the stage's white
mat on the paper ground reads faintly outside the quiet view — mat/bg
1.02:1, ΔL 0.01 — an interim state stated and accepted. At the gate the
photographer went one step further — "having the matte only on the 'quiet
view' or whenever the background is darkened is the right move as well";
asked, "stage's normal view unmatted, as it blends into the Paper ground
tone" — which is a spec change, so it went to `ROADMAP.md` ("The mat only
where the ground goes dark") rather than into this spec's implementation.

**The ground (2026-09-19, both screens, the site unmatted under the dev
server).** "Looks great and I think this confirms that removing the mattes
in most cases was the right move. I'd say we'd go with Paper for the ground
tone for now." The tone is `paper`, `oklch(0.99 0.003 100)` — the pre-003
white — and the family at 014's fixed steps from it: surface
`oklch(0.967 0.004 100)`, soft `oklch(0.942 0.005 100)`, line
`oklch(0.882 0.005 100)`, line-strong `oklch(0.742 0.009 100)`. The readout
at paper: text/bg 15.58, text/surface 14.55, text/soft 13.51, muted/bg 5.82,
muted/surface 5.44, muted/soft 5.05; the lowest pair, muted on the soft fill,
clears the 4.5:1 floor, so the muted text ships **unchanged** at
`oklch(0.5 0.012 250)` — this gate deepened nothing. The unmatted surfaces
were seen on both screens as the spec describes them and nothing was
reported that the spec did not say. The derived copies followed by 014's
rule: `COLOR.bg` in `src/lib/og-card.mjs` is `#fcfcfa` and `public/og.jpg`
was regenerated by `npm run og` and shown at the Phase 1 pause. "For now"
is his — the tone is the one the pages have today, not a promise.

**Spec 014's machinery was built and its gate closed unanswered.** Its
Phase 0 — the family rule and contrast floor in `src/lib/ground.ts`, the
dev-only site-wide switch, the sampler's candidate bar, free tune and
readout, and the social image's generator — shipped green and is exactly
what this gate ran on; `CANDIDATES.today` now follows the landed ground and
`warm-003` carries spec 003's warm as the control 014's spread test measures
against. Its Phases 1 and 2 never ran: the gate they were written for was
overtaken by the question about the mats, and its `plan.md` and `tasks.md`
say so. The sampler's mat bar now moves the stage section alone, since a
bar on an unmatted surface would move nothing but the deferred pause frame.

**The aspect-ratio follow-up needs a rethink.** Its likely answer — frames
of different ratios set in mats of one ratio so they line up side by side
— assumed matted gallery frames, and this spec removed them; the product
